...
QtObject {
    ...
    property string __query__fetch__: 'SELECT * FROM %1 WHERE status = 0'.arg(tableName)
    ...
    function sqlCreate(data) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            for(let i = 0; i < value.length; i++) {
                let v = column[i]
                for(let u = 0; u < control.column.length; u++) {
                    let uu = control.column[u]
                    if(uu['name'] === v) {
                        if(uu['type'] === "TEXT") {
                            completer = completer.replace('?', "'%1'".arg(value[i].toString()))
                        }else {
                            completer = completer.replace('?', "%1".arg(value[i].toString()))
                        }
                    }
    ...
    function sqlCreates(datas) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            for (var b = 0; b < datas.length; b++) {
                ...
                for(let i = 0; i < vv.length; i++) {
                    ...
                    for(let u = 0; u < control.column.length; u++) {
                        ...
                        if(uu['name'] === v) {
                            if(uu['type'] === "TEXT") {
                                finalString = finalString.replace('?', "'%1'".arg(vv[i]))
                            }else {
                                finalString = finalString.replace('?', "%1".arg(vv[i]))
                            }
                        }
            ...
            var rs = control.db.executeSql(logQuery(__query__creates__.arg(
                control.tableName).arg(
                column_str).arg(
                finalString)), value)
            resolve(rs)
    ...
    function sqlDelete(id) {
        return new Promise(function(resolve, reject) {
            let query = 'DELETE FROM %1 WHERE id= %2'.arg(control.tableName).arg(id)
    ...
    function init() {
        return new Promise(function(resolve, reject) {
            ...
            resolve(control.db.executeSql(logQuery(query)))
        })
    }
